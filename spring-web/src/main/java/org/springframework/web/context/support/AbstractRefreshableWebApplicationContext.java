...
public abstract class AbstractRefreshableWebApplicationContext extends AbstractRefreshableConfigApplicationContext
    ...
    //设置AbstractApplicationContext的displayName
	 ...
	 */
	//注册web特定的作用域
	//这里重点是下面的两行代码，注册Web特定的作用域（“request”, “session”, “globalSession”, “application”）
	// 至BeanFactory中，方便WebApplicationContext使用，作用域这里可以先这样理解（从Bean的创建角度来理解）:
	//request 为每个Http请求创建一个Bean实例
	//session 为Http请求的会话创建一个Bean实例
	//globalSession 为全局Http会话创建一个Bean实例
	//application 为整合web应用创建一个Bean实例
	@Override

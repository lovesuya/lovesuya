...
public class ContextLoader {
	...
	static {
		...
		//从ContextLoader.properties文件中，加载默认的webapplicationContext的实现类
	 ...
	 */
	/**
	 * WebApplicationContext根据在context-params中配置contextClass和contextConfigLocation完成初始化
	 */
	public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
		//判断servletContext对象中是否存放了WebApplicationContext对象
		...
		try {
			...
			if (this.context == null) {
				//返回的是ConfigurableWebApplicationContext接口类型的对象
				//具体的实现没有配置，默认则是XmlWebApplicationContext
				//XmlWebApplicationContext 实现了ConfigurableWebApplicationContext
				this.context = createWebApplicationContext(servletContext);
			}
			//返回的就是ConfigurableWebApplicationContext类型
	...
	protected WebApplicationContext createWebApplicationContext(ServletContext sc) {
		//此方法很重要,获取争取真正的WebApplicationContext实现类,完成接下来的容器操作
		//如果web.xml中配置了contextClass，则使用配置的，否则使用默认的
		Class<?> contextClass = determineContextClass(sc);
		if (!ConfigurableWebApplicationContext.class.isAssignableFrom(contextClass)) {
			throw new ApplicationContextException("Custom context class [" + contextClass.getName() +
					"] is not of type [" + ConfigurableWebApplicationContext.class.getName() + "]");
		}
		//XmlWebApplicationContext实现了ConfigurableWebApplicationContext
	...
	protected Class<?> determineContextClass(ServletContext servletContext) {
		...
		if (contextClassName != null) {
			...
			//使用默认的WebApplicationContext实现类 XmlWebApplicationContext
